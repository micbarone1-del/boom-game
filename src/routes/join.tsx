  ...
  head: () => ({
    meta: [
      { title: "Join the Game — BOOM!" },
      {
        name: "description",
        content:
          "Join a BOOM! workout game from your phone. Enter your room code, set your fitness level, and get ready to roll.",
      },
      { property: "og:title", content: "Join the Game — BOOM!" },
      {
        property: "og:description",
        content:
          "Join a BOOM! workout game from your phone. Enter your room code and play with friends.",
      },
...
function JoinPage() {
  ...
  const deleteSavedProfile = async () => {
    if (!userId) return;
    if (
      !confirm("Delete your saved BOOM profile? Your name, avatar and fitness level will be wiped.")
    )
      return;
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setError(null);
    if (!code || !username) {
      setError("Need a code and a name.");
      return;
    }
    setSubmitting(true);
    try {
      ...
      const [{ data: room }, { data: roomPlayers }] = await Promise.all([
        ...
        supabase
          .from("players")
          .select("id, fitness_level, joined_at, team_id, is_team_lead, current_space")
          .eq("room_code", code)
          .order("joined_at"),
      ]);
      if (!room) {
  ...
  return (
    <main className="min-h-screen p-6 flex flex-col items-center">
        ...
        {userId ? (
          <>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={signOut}
                className="text-xs font-black flex items-center gap-1 underline"
              >
          ...
          <>
            <span className="text-xs font-bold text-foreground">
              Sign in to save your profile across games.
            </span>
            <button
      ...
      {showAuth && !userId && (
        <form
          onSubmit={handleAuth}
          className="ink-border rounded-2xl bg-white p-4 w-full max-w-md mb-3 flex flex-col gap-2"
        >
          <div className="flex flex-col gap-2 mb-1">
            ...
            >
              <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden>
                <path
                  fill="#FFC107"
                  d="M43.6 20.5H42V20H24v8h11.3c-1.6 4.6-6 8-11.3 8-6.6 0-12-5.4-12-12s5.4-12 12-12c3 0 5.7 1.1 7.8 3l5.7-5.7C33.9 6.1 29.2 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.2-.1-2.3-.4-3.5z"
                />
                <path
                  fill="#FF3D00"
                  d="M6.3 14.7l6.6 4.8C14.6 16.1 19 13 24 13c3 0 5.7 1.1 7.8 3l5.7-5.7C33.9 6.1 29.2 4 24 4 16.3 4 9.6 8.4 6.3 14.7z"
                />
                <path
                  fill="#4CAF50"
                  d="M24 44c5.2 0 9.9-2 13.4-5.2l-6.2-5.2c-2 1.4-4.5 2.4-7.2 2.4-5.3 0-9.7-3.4-11.3-8l-6.5 5C9.5 39.5 16.2 44 24 44z"
                />
                <path
                  fill="#1976D2"
            ...
            <div className="text-center text-[10px] font-bold text-foreground/80">
              — or use email —
            </div>
          </div>
          ...
          />
          {authMsg && (
            <div className="text-xs font-bold" style={{ color: "var(--boom-red)" }}>
              {authMsg}
            </div>
          )}
      ...
      <form
        onSubmit={submit}
        className="ink-border rounded-3xl bg-white p-6 w-full max-w-md flex flex-col gap-4"
      >
          ...
          {(preview || savedAvatar) && (
            <div className="mt-2 flex items-center gap-3">
              <div className="w-24 h-24 rounded-full overflow-hidden ink-border-sm">
                <img
                  src={preview ?? savedAvatar ?? ""}
                  alt={`${username || "Player"} selfie preview`}
                  className="w-full h-full object-cover"
                />
              </div>
              <button
                type="button"
                onClick={clearProfileFields}
                className="text-xs font-black underline"
                style={{ color: "var(--boom-red)" }}
              >
                Clear
